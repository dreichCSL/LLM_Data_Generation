...
def run_generation(parameter_dict, cfg):
    # include parameters
    parameter_dict.update({'llm_sampling_params': cfg.llm_gen_config.inference_config.llm_sampling_params.model_dump(),
                           'vllm_engine_params': cfg.llm_gen_config.inference_config.vllm_engine_params.model_dump(),
    ...
    logger.info(f"Initializing inference engine with settings:\n{model_specs}")
    generator = get_llm_generator(model_specs)
    
    # run generation
    gen_specs = GenRun(**parameter_dict)
    logger.info(f"Generating with LLM settings:\n{gen_specs}")
...
def generate_questions(config_yaml, output_dir):
    ...
    dataloader = DataLoader(
        dataset,
        batch_size=cfg.llm_gen_config.inference_config.batch_size_dl,
        shuffle=False,
        drop_last=False
    )
...
def generate_answers(config_yaml, output_dir):
    # config
    cfg = load_config(config_yaml)
    
    # output
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, "gen_answers.jsonl")

    # init dataset and dataloader
    dataset = TextAndDiscourseConditioned(cfg.input_paths.context_file, 
                                          cfg.input_paths.discourse_file,  # contains sample_id field ("contextId_discourseActId")
                                          cfg.llm_gen_config.model_setup.prompt_templates.model_dump())
    dataloader = DataLoader(
        dataset,
        batch_size=cfg.llm_gen_config.inference_config.batch_size_dl,
    ...
    llm_response_processor = get_llm_response_processor("answer")
