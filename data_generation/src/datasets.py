...
# class AnswerGenDataset(Dataset):
    """Dataset for answer generation for given questions and passages."""
    ...
    def __getitem__(self, index):
        ...
        context_id = data['sample_id'].split('_')[0]  # sample_id consists of text_id + question_idx, all strings
        context = self.text_data[context_id]
        language = self.text_data[context_id].get('language', "en")
        placeholder_content = {"user": {"passage": context, "discourse_act": data['text']}}
        prompt = assemble_prompt(self.prompt_templates, placeholder_content, language)
